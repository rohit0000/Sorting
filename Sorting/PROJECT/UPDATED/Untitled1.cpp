...
int main()
{
	...
	int i, x, temp_int, stopper=0, login_ID, user_or_admin=0, identity, edit_user, edit_smartphone, shop[20][20], counter=0;
	char temp_string[129], temp_description[200], login_password[129];
	int log_in, pickloop=0, index=-1, approver=0, temp, transaction, admin_operation, cart_holder[100], edit_account;
	...
	ptr_file_shoppingcart = fopen("Shopping Cart.txt", "r");
...
for(i=0; i<20; i++){
	...
	for(i=0; i<20; i++){
		...
		for(x = 0; x<20; x++){
		fscanf(ptr_file_shoppingcart, "%d", &shop[i][x]);
		user_array[i].shoppingcart[x] = shop[i][x];
		}
		...
		if(log_in==1){
			...
			if(user_or_admin==1){
				...
				do{
					approver=0;
					counter=0;
                    ptr_file_shoppingcart = fopen("Shopping Cart.txt", "w");
					pickloop=0;
					system("cls");
					...
					switch(transaction)
					{
						case 1:{
                            ...
                            if(admin_operation == 1){
                                system("cls");
                                printf("Please input the smartphone number you want to add: ");
                                scanf("%d", &temp_int);
                                temp_int = temp_int -1;
                                for(x = 0; x<20; x++){
                                if(user_array[identity].shoppingcart[x] == -1){
                                    user_array[identity].shoppingcart[x] = temp_int;
                                    user_array[identity].money = user_array[identity].money - smartphone_array[temp_int].price;
                                    printf("Smartphone #%d added\n You now have P%d !\n", temp_int+1, user_array[identity].money);
                                    x = 20;
                                }
                            }
                            system("pause");
						...
						case 2:{
							...
							printf("Please choose a category to be searched:\n\n");
                            printf("[1] - Name\n");
                            printf("[2] - Brand\n");
                            printf("[3] - Price\n");
                            scanf("%d", &temp_int);
                            if(temp_int == 1){
                                system("cls");
                                printf("Please input the name of the smartphone:");
                                getchar();
                                gets(temp_string);
                                strcat(temp_string, "\n");
                                for(i=0; i<100; i++){
                                    if((strcmp(smartphone_array[i].name, temp_string)) == 0){
                                        printf("\t\t\t\tSMARTPHONE #%d\n\n", i+1);
                                        printf("Name: %s\n", smartphone_array[i].name);
                                        printf("Brand: %s\n", smartphone_array[i].brand);
                                        printf("Price: P%d\n", smartphone_array[i].price);
                                ...
                                if(approver == 0){
                                	...
                                	printf("Please enter the Smartphone number that you want to add:");
                                	scanf("%d", &temp);
                                	if(temp>=1){
                                		temp=temp-1;
                                		for(i=0; i<20; i++){
                                			if(user_array[identity].shoppingcart[i]==-1){
                                				user_array[identity].shoppingcart[i]=temp;
                                				user_array[identity].money = user_array[identity].money - smartphone_array[temp].price;
                                				i=20;
                                			}
                                		}
                                		printf("You have successfully added smartphone #%d to your shpooing cart!\n\nYou now have a credit of P%d! ", temp+1, user_array[identity].money);
                                	}
                                	else{
                            ...
                            if(temp_int == 2){
                                system("cls");
                                printf("Please input the brand of the smartphone:");
                                getchar();
                                gets(temp_string);
                                strcat(temp_string, "\n");
                                for(i=0; i<100; i++){
                                    if(strcmp(smartphone_array[i].brand, temp_string) == 0){
                                        printf("\t\t\t\tSMARTPHONE #%d\n\n", i+1);
                                        printf("Name: %s\n", smartphone_array[i].name);
                                        printf("Brand: %s\n", smartphone_array[i].brand);
                                        printf("Price: P%d\n", smartphone_array[i].price);
                            ...
                            if(temp_int == 3){
                                system("cls");
                                printf("Please input the max price of the smartphone:");
                                scanf("%d", &temp_int);
                                for(i=0; i<100; i++){
                                    if(temp_int >= smartphone_array[i].price){
                                        printf("\t\t\t\tSMARTPHONE #%d\n\n", i+1);
                                        printf("Name: %s\n", smartphone_array[i].name);
                                        printf("Brand: %s\n", smartphone_array[i].brand);
                                        printf("Price: P%d\n", smartphone_array[i].price);
						...
						case 3:{
                            ...
                            printf("My Name: %s\n", user_array[identity].name);
                            printf("My ID: %d\n", user_array[identity].id);
                            printf("My Password: %s\n", user_array[identity].password);
                            printf("My account money: P%d\n", user_array[identity].money);
                            printf("My smartphones:\n");
                            i=0;
                            for(x=0; x <20; x++){
                                if(user_array[identity].shoppingcart[x] != -1){
                                	temp = user_array[identity].shoppingcart[x];
                                	cart_holder[i]= x;
                                    printf("%d.) %s\n", i+1, smartphone_array[temp].name);
                                    i++;
                                }
                            }
                            if(i==0){
                            	printf("You do not own any smartphones yet.\n\nWant to purchase smartphones? Visit the smartphone database!\n\n");
                            }
                            printf("\n[1] - Remove a smartphone from shopping cart\n[2] - Edit my ID\n");
                            printf("[3] - Edit my Name\n[4] - Edit my Password\n[5] - Go Back\n");
                            scanf("%d", &edit_account);
                            if(edit_account==1){
                            	if(i!=0){

                            		printf("\n\nFrom the list above, please enter the number of the smartphone that you want to remove:");
                            		scanf("%d", &temp);
                            		temp = temp-1;
                            		x = cart_holder[temp];
                            		temp = user_array[identity].shoppingcart[x];
                            		user_array[identity].shoppingcart[x] = -1;
                            		user_array[identity].money = user_array[identity].money + smartphone_array[temp].price;
                            		printf("You have successfully removed a smartphone from your shopping cart!\n\nYou now have a credit of P%d! ", user_array[identity].money);

                            	}
                            	else{
                            		printf("\nI'm sorry but you do not have any smartphone in your cart!\n");
                            	}
                            }
                            else if(edit_account==2){
                            	printf("Enter a new ID:");
                            	scanf("%d", &temp);
                            	for(i=0; i<20; i++){
                            		if(temp==user_array[i].id){
                            			counter++;
                            		}
                            	}
                            	if((temp>=100)&&(counter==0)){
                            			user_array[identity].id = temp;
                            			printf("You have successfully editted your ID! ");
                            	}
                            	else{
                            		system("cls");
                            		printf("I'm sorry. Either your entry ID has already been taken or it is invalid!\n");
                            	}
                            }
                            else if(edit_account==3){
                            	printf("Enter a new Name:");
                            	getchar();
                            	gets(user_array[identity].name);
                            	printf("You have successfully editted your name! ");
                            }
                            else if(edit_account==4){
                            	printf("Enter a new Password:");
                            	getchar();
                            	gets(user_array[identity].password);
                            	printf("You have successfully editted your password! ");
                            }
                            else if(edit_account==5){
	...
	for(i=0; i<20; i++){
		...
		fprintf(ptr_file_username, "%s", user_array[i].name);
		fprintf(ptr_file_userpassword, "%s", user_array[i].password);
		for(x = 0; x<20; x++){
		fprintf(ptr_file_shoppingcart, "%d ", user_array[i].shoppingcart[x]);
		}
		fprintf(ptr_file_shoppingcart, "\n");
	}
	for(i=0; i<5; i++){
	...
	for(i=0; i<100; i++){
		fprintf(ptr_file_smartphonename, "%s", smartphone_array[i].name);
		fprintf(ptr_file_smartphonebrand, "%s", smartphone_array[i].brand);
		fprintf(ptr_file_smartphoneprice, "%d\n", smartphone_array[i].price);
		fprintf(ptr_file_smartphonedescription, "%s", smartphone_array[i].description);
	}
